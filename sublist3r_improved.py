...
# 11.12.2023 03:08:18 Basty-devel
...
def banner():
    print("""%s    _____                                    _____                                                               
 __|_    |__   ____   ____   _   __  __   __|___  |__   _____    ____     __  __  __   ____     ______   _____   
|    |      | |    | |    \ | | |  |/ /  |   ___|    | |     |  |    \   |  \/  \|  | |    |   |___   | |     |  
|    |_     | |    | |     \| | |     \  |   |__     | |     \  |     \  |     /\   | |    |_  |___   | |     \  
|______|  __| |____| |__/\____| |__|\__\ |______|  __| |__|\__\ |__|\__\ |____/  \__| |______| |______| |__|\__\ 
   |_____|                                  |_____|                                                              
                                                                                                                 

                # based on aboul3la's Sublist3r + HakLuke's Hakrawler
                # by Basty-devel
    """) 
...
def parse_args():
    ...
    parser.add_argument('-b', '--bruteforce', help='Enable the subbrute bruteforce module', nargs='?', default=True)
    parser.add_argument('-p', '--ports', help='Scan the found subdomains against specified tcp ports')
    parser.add_argument('-v', '--verbose', help='Enable Verbosity and display results in realtime', nargs='?', default=False)
    parser.add_argument('-t', '--threads', help='Number of threads to use for subbrute bruteforce', type=int, default=40)
    parser.add_argument('-e', '--engines', help='Specify a comma-separated list of search engines')
    parser.add_argument('-o', '--output', help='Save the results to text file')
    parser.add_argument('-n', '--no-color', help='Output without color', default=True, action='store_true')
...
#                        'DuckDuckBot/1.0',
#                        'ia_archiver (+http://www.ask.com/ia_archiver.html)',
#                        'Gigabot/1.0',
#                        'Sogou web spider/4.0 (+http://www.sogou.com/docs/help/webmasters.htm#07)',
#                        'NaverBot/1.0 (+http://search.naver.com/search.naver)',
#                        'AOLBot/1.0'
                        # feel free to add more bot-engines, this is can be more effective with AnnonScript, Pi Onion, VPN(selfhosted), Proxychains etc.
#                        ])
...
class enumratorBase(object):
    ...
    def print_banner(self):
        """ """
        self.print_("[-] Searching now in %s.." % (self.engine_name))
        return
        ...
        try:
            ...
            for link in links_list:
                ...
                if subdomain and subdomain not in self.subdomains and subdomain != self.domain:
                    if self.verbose:
                        self.print_("%s%s: %s%s" % (self.engine_name,  subdomain))
...
class DNSdumpster(enumratorBaseThreaded):
    ...
    def check_host(self, host):
        ...
        try:
            ...
            if ip:
                if self.verbose:
                    self.print_("%s%s: %s%s" % (self.engine_name, host))
                is_valid = True
...
class portscan():
    ...
    def port_scan(self, host, ports):
        ...
        self.lock.release()
        if len(openports) > 0:
            print("%s%s%s - %sFound open ports:%s %s%s%s" % (host, ', '.join(openports)))
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if not silent:
        print("[-] Enumerating subdomains now for %s" parsed_domain.netloc)
    ...
    def save_subdomains(subdomains, safefile=args.output, savefile=True, silent=False, ports=None):
        if subdomains:
            subdomains = sorted(subdomains, key=lambda x: (x.split('.')[-2:], x.split('.')))

            if savefile:
                try:
                    with open(safefile, 'w') as f:
                        for subdomain in subdomains:
                            f.write(subdomain + '\n')
                    return safefile
                except Exception as e:
            ...
            if not silent:
                print("[-] Total Unique Subdomains Found: %i" % len(subdomains))

            if ports:
                ports = ports.split(',')
                if not silent:
                    print("[-] Start port scan now for the following ports: %s" % (ports))
    ...
    save_subdomains(subdomains_list, safefile=args.output, savefile=True, silent=False, ports="21,22,53,80,443")
